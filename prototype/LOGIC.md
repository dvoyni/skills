...
A single, self-contained HTML file (a **shareable demo**) that lets anyone drive a state model by clicking buttons. Use this when the question is about **rules, state transitions, or data shape**: business logic, a game economy, a progression curve, a loot table. The kind of thing that looks reasonable on paper but only feels wrong once you push it through real cases.
...
- "Does this economy hold up over fifty turns, or does gold run away?"
- "Can this loot table actually produce the drop I want?"
- Anything where someone wants to **press buttons and watch state change**.

If the question is "what should this look like," use [UI.md](UI.md). If it is how a mechanic feels in the hand, use [FEEL.md](FEEL.md): feel is answered with a controller, not with buttons.
...
Write it for a non-developer. Every label is in **domain language**, not code: buttons and state read like the domain (the business, the game), not the reducer. Explain in plain words what's happening.
...
Once the prototype has answered its question, capture the answer, then capture the prototype the way the [SKILL](SKILL.md) describes. The logic-specific mapping: the validated reducer / machine / function set lifts into the real module (the decision, absorbed); the HTML shell rides along to whichever capture the SKILL selects, and being one self-contained file, it stays trivially re-runnable wherever it lands.
